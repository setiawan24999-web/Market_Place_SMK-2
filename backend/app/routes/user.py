...
from app.routes.auth import acak_password ,feryfikasi_pasword, buatkan_hak_akses
...
def login(user_data: UserLogin, db:Session=Depends(get_sesion)):
    ...
    acces_token = buatkan_hak_akses(data={"sub":user.username})
...
def get_my_profile(current_username: str = Depends(get_current_user), db: Session = Depends(get_sesion)):
    ...
    if not user2:
        # Gunakan status code yang standar (404 atau 401)
